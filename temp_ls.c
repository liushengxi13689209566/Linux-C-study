...
int jiexi(char *pathname)
{
    int i,j,len ;
    char name[NAME_MAX + 1];    //NAME_MAX     一个单独文件最长的名字
    len=strlen(pathname);
    for(i= 0,j= 0;i< len;i++)  //作用：把pathname (/home/liushengxi/****) 的文件名****存储于name 中
    {
        if(pathname[i]=='/')
        {
            j= 0;
...
void display_dir(int  flag_param,char *path)  //path 目录名，即：/home/liushengxi/***/    或者是   /
{
    ...
    char **filename ,*temp;
    filename=(char **)malloc(sizeof(char *) * count );
    for(i= 0 ;i< count ;i++)
        filename[i]=(char *)malloc(sizeof(char)*(PATH_MAX + 1) );
    ...
    for(i= 0;i< count- 1;i++)  //冒泡排序文件名, 考虑-r 参数
    {
        for(j= 0;j< count -1 -i ;j++)
        {
            if(flag_temp ) //-r
            {
                if(strcmp(filename[j],filename[j+ 1]) < 0)//先后顺序不能变
                {
    ...
    if( flag_temp1 )
    {
        for(i= 0;i < count ; i++ )
        {
            if(lstat(filename[i],&buf)< 0 ) //有-R 参数才判断
            {
                printf("Sorry !! you have no power to open %s!!!\n",filename[i]);
                continue ;
            }
            if(S_ISDIR(buf.st_mode) && jiexi(filename[i]) )  
            {
                printf("*****************************************%s\n",filename[i]);
                strcat(filename[i],"/\0");
