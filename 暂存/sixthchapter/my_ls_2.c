...
int InQueue(Link_queue *p_link,char *name_queue ); //入队列
Link_queue *init_queue() ;
int  OutQueue(int flag_param,char *pathname,Link_queue *p_link) ; // 出队列，返回
void display(int flag_param,char *pathname,Link_queue *p_link);  ///home/liushengxi/test.c   /home/liushengxi/.vimrc  处理一个目标文件
...
void display_dir(int  flag_param,char *path)  //path 目录名，即：/home/liushengxi/***/    或者是   /
{
    ...
    if(flag_temp2)    link_queue_temp  = init_queue(); 
    ...
    for(i= 0;i< count ; i++ )
    {
        if(flag_temp2)    //存在-R 参数,入队列
        {
            ...
            if(S_ISDIR(buf.st_mode))     InQueue(link_queue_temp,filename[i]);
        }
        display(flag_param,filename[i],link_queue_temp);//多次调用函数
    }
    closedir(dir); 
}
...
int  OutQueue(int flag_param,char *pathname,Link_queue *p_link)  // 出队列，返回
{
    ...
    {
        if(display_dir(flag_param,pathname ,p_link->front->file_name_queue)  < 0)
...
void display(int flag_param,char *pathname,Link_queue *p_link)  ///home/liushengxi/test.c   /home/liushengxi/.vimrc  处理一个目标文件
{
    ...
    flag_temp3=flag_param ;
    flag_temp3=flag_temp3 >> 3  & 1; //取出第三位,代表 -R 参数

    k= OutQueue(p_link);        //判断是否出错,队空或者是打不开的目录就都返回 -1

    if(flag_temp3 && k)  
        OutQueue(flag_param ,pathname,Link_queue *p_link);   //出队列
